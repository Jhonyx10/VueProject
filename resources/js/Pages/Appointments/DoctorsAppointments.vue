...
<template>
    <div class="space-y-6">
        <div class="flex items-center justify-between px-2">
            <div>
                <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Patient Appointments</h2>
                <p class="text-sm text-slate-500">You have {{ mySchedule.length }} consultations scheduled</p>
            </div>
            <div class="flex gap-2">
                <div class="bg-white border border-slate-200 px-4 py-2 rounded-2xl shadow-sm text-xs font-bold text-slate-600">
                    Today's Date: {{ new Date().toLocaleDateString() }}
                </div>
            </div>
        </div>

        <div v-if="mySchedule.length === 0" 
            ...
            <p class="text-slate-500 max-w-xs mx-auto">When patients book appointments with you, they will appear here.</p>
        </div>

        <div v-else class="grid gap-4">
            <div v-for="app in mySchedule" :key="app._id || app.id" 
                ...
                <div class="flex flex-row md:flex-col items-center justify-center bg-slate-50 px-6 py-4 rounded-[2rem] min-w-[120px] gap-2 border border-slate-100">
                    <span class="text-indigo-600 font-black text-lg">{{ formatTime(app.time) }}</span>
                ...
                <div class="flex-1 bg-slate-50/50 p-4 rounded-2xl border border-slate-50">
                ...
                <div class="flex items-center gap-3">
                    <button class="bg-indigo-50 text-indigo-600 p-3 rounded-2xl hover:bg-indigo-100 transition-colors group-hover:scale-105">
