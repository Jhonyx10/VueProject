...
<template>
    <div class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div
                v-for="(stat, index) in [
                    { label: 'Total Bookings', count: appointments.length, color: 'text-slate-800', iconColor: 'text-indigo-600' },
                    { label: 'Pending Bookings', count: pendingCount, color: 'text-amber-600', iconColor: 'text-amber-600' },
                    { label: 'Completed Bookings', count: completedCount, color: 'text-green-800', iconColor: 'text-green-600' }
                ]"
                :key="stat.label"
                v-motion
            ...
            >
                <div :class="['w-12 h-12 bg-indigo-50 rounded-2xl flex items-center justify-center', stat.iconColor]">
                    <CalendarIcon class="w-6 h-6" />
                </div>
                <div>
                    <p :class="['text-xs font-bold uppercase', stat.iconColor === 'text-indigo-600' ? 'text-slate-400' : stat.iconColor]">
                        {{ stat.label }}
                    </p>
        ...
        >
                    ...
                    <tr>
                        ...
                        <th class="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-slate-400">
                            <div class="flex items-center gap-4">
                                <span>Status</span>
                                <select v-model="statusQuery" class="ml-2 p-2 bg-transparent border-none text-[10px] font-black uppercase tracking-widest text-indigo-600 focus:ring-0 cursor-pointer">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    >
                        ...
                        <td class="px-8 py-5 text-sm text-slate-600">
                            <div class="flex flex-col">
                                <span class="font-bold text-slate-800">Dr. {{ app.doctor?.doctor_profile?.lastName }}</span>
                                <span class="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">{{ app.doctor?.doctor_profile?.expertise }}</span>
                            </div>
                        </td>
                            ...
                            <span
                                :class="{
                                    ...
                                    'bg-red-50 text-red-600 border-red-100': app.status === 'cancelled',
                                     'bg-red-50 text-red-600 border-red-100': app.status === 'expired',
                                }"
            ...
            <div
                ...
                class="px-8 py-4 bg-slate-50/50 border-t border-slate-100 flex items-center justify-between"
            >
                ...
                <div class="flex items-center gap-2">
                    <button @click="currentPage--" :disabled="currentPage === 1" class="p-2 rounded-xl border border-slate-200 bg-white text-slate-600 disabled:opacity-30 hover:bg-slate-50 transition-colors">
                    ...
                    <div class="flex items-center gap-1">
                        <button
                            ...
                            :class="[
                                ...
                                currentPage === page ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50',
                            ]"
                        >
                            {{ page }}
                        </button>
                    ...
                    <button @click="currentPage++" :disabled="currentPage === totalPages" class="p-2 rounded-xl border border-slate-200 bg-white text-slate-600 disabled:opacity-30 hover:bg-slate-50 transition-colors">
