<template>
    <div class="flex min-h-screen bg-slate-50 font-sans antialiased text-slate-900">
        ...
        <aside :class="isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
            ...
            <div class="h-20 flex items-center justify-between px-8">
                <Link :href="route('dashboard')" class="flex items-center gap-3 group">
                    ...
                    <span class="font-bold text-xl tracking-tight text-slate-800">MedCare</span>
                </Link>
                <button @click="isMobileMenuOpen = false" class="lg:hidden p-2 text-slate-500">
         ...
         <NavLink :href="route('chat.index')" :active="route().current('chat.index')"
            ...
            <div class="p-4 border-t border-slate-100">
                <div class="bg-slate-50 rounded-2xl p-4 flex items-center gap-3">
                    <div class="h-10 w-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-bold border-2 border-white shadow-sm shrink-0">
        ...
        <div class="flex-1 flex flex-col transition-all duration-300 lg:ml-64 w-full">
            <header class="h-20 flex items-center justify-between px-4 sm:px-8 bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-10">
                <div class="flex items-center gap-4">
                    <button @click="isMobileMenuOpen = true" class="lg:hidden p-2 text-slate-600 hover:bg-slate-100 rounded-xl transition-colors">
                        <Bars3Icon class="h-6 w-6" />
                    </button>
                    <h2 class="text-lg font-semibold text-slate-800 capitalize hidden sm:block">
                    ...
                    <Dropdown align="right" width="48">
                        <template #trigger>
                            <button class="flex items-center gap-1 p-1 hover:bg-slate-100 rounded-lg transition-colors">
                                <div class="h-8 w-8 bg-slate-200 rounded-lg flex items-center justify-center text-slate-600">
                        ...
                        <template #content>
                            ...
                            <DropdownLink :href="route('logout')" method="post" as="button" class="text-red-600 w-full text-left">Log Out</DropdownLink>
                        </template>
                    </Dropdown>
                </div>
            </header>
